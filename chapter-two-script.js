...
export const CHAPTER_TWO_STEPS = [
  ...
  {
    id: 'bells', map: 'plaza', where: 'LANTERN PLAZA',
    title: 'Bells for a missing dawn',
    description: 'Take the east bridge to Lantern Plaza. Read the drowned bell-ringer\'s verse and sound the three bells in order.',
  ...
  },
  {
    ...
    description: 'Follow the south bridge to Skibidi Yard. Close all three pressure valves within 45 seconds. The timer starts when you close the first valve.',
    recap: 'You closed the three valves before pressure returned and freed the last note from the old pumping yard.',
  },
  {
    id: 'vigil', map: 'plaza', where: 'LANTERN PLAZA',
    title: 'Keep the last light burning',
    description: 'Return to the meridian beacon in Lantern Plaza. Kindle it and survive three waves of drowned echoes: two, three, then four.',
  ...
  },
  {
    ...
    description: 'Return to Skibidi Yard and challenge the Hollow Warden. Jump over its red pulse or move clear, then strike while its shield is down to free the Tidewarden\'s stolen voice.',
...
export function chapterTwoConversation(person, state = {}) {
  ...
  if (person === 'chart') {
    ...
    if (state.accepted) return { lines: [
      say('chart', 'Root, bell, iron. Pine Islet, Lantern Plaza, the pumping yard. Three locks on one stolen voice.'),
      say('chart', state.warden ? 'Bring its voice to the seal beside the Moonwell. Let the whole harbour hear it.' : 'When all three locks open, kindle the meridian beacon in the plaza. Then face the Hollow Warden in the pumping yard and free the Tidewarden\'s voice.'),
    ] };
    return { sets: 'accepted', lines: [
      ...
      say('chart', 'The Long Tide stole its voice and sealed it in drowned iron. Three locks hold it: a memory under Pine Islet, a song in Lantern Plaza, and a breath in the old pumping yard.'),
      say('you', 'And if I open them?'),
      say('chart', 'Kindle the plaza\'s meridian beacon. It will reveal the Hollow Warden in the pumping yard: an iron shell with our guardian\'s voice trapped inside. Break that shell and bring the voice home.'),
  ...
  if (person === 'beacon') {
    ...
    if (state.vigil) return narration('The beacon holds steady. Its light reveals the Hollow Warden in the pumping yard. Cross the south bridge and break the iron shell to free the Tidewarden\'s voice.');
    return narration(
  ...
  if (!FLAGS.slice(0, -1).every(flag => state[flag] === true)) return narration('The seal has a place for a voice, but no voice to carry. Open the three locks, survive the plaza\'s vigil, then break the Hollow Warden in Skibidi Yard to free the Tidewarden\'s voice.');
  ...
  return { sets: 'complete', lines: [
    ...
    say(null, 'Across Pine Islet, the roots shine silver. Lantern Plaza rings for dawn. In the yard, the old pumps breathe with the tide instead of against it.'),
