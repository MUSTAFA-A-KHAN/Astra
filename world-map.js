...
import { PLAZA_STEP, PLAZA_TRANSFORM, loadPlazaDistrict } from './plaza-world.js';
...
// container pier off its south-east corner. Two more join them only when the
// player turns them on: the Nightwood's road off the northern quay, across the
// city from the yard, and the Red Mesa off the southern quay, west of the
// yard. This module places them, joins them with jetties, and hands the game a
// single piece of ground that spans them all.
...
export async function createWorld(scene, { lowPower = false, nightwood = false, mesa = false } = {}) {
  const [city, forest, plaza, yard, wood, butte] = await Promise.all([
    ...
    // Until the player turns the Nightwood or the mesa on, neither its module
    // nor its model is fetched, and nothing below makes room for it.
  ...
  const jetty = createForestJetty(forest), eastJetty = createPlazaJetty(plaza), southJetty = createYardJetty(yard), northJetty = wood && createNightwoodJetty(wood), mesaJetty = butte && createMesaJetty(butte);
  const crossings = [jetty, eastJetty, southJetty, ...(northJetty ? [northJetty] : []), ...(mesaJetty ? [mesaJetty] : [])];
  root.add(...crossings.map(crossing => crossing.group));
  const districts = [city.bounds, forest.bounds, plaza.bounds, yard.bounds, ...(wood ? [wood.bounds] : []), ...(butte ? [butte.bounds] : [])];
  const bounds = {
  ...
  const navigation = createNavigation([city.terrain, forest.terrain, plaza.terrain, yard.terrain, ...(wood ? [wood.terrain] : []), ...(butte ? [butte.terrain] : []), ...crossings.map(crossing => crossing.terrain)], bounds,
  ...
  const plazaReachable = navigation.isWalkable(eastJetty.inland.x, eastJetty.inland.z, 1);
  ...
  const shardPositions = [...cityShards, ...(forestReachable ? forestShards : [[-65,-10],[66,-48],[-25,60],[25,48]]), ...(plazaReachable ? plazaShards : []), ...(yardReachable ? yardShards : []), ...optional(woodReachable, woodShards), ...optional(mesaReachable, mesaShards)];
  ...
  const diagnostics={ready:true,provider:'astra-world-map',asset:city.asset,assets:[city.asset,forest.asset,plaza.asset,yard.asset,...(wood?[wood.asset]:[]),...(butte?[butte.asset]:[])],
  ...
  return {
    ...
    biomeAt(x,z) {
      return within(forest.bounds,x,z) ? 'forest' : within(plaza.bounds,x,z) ? 'plaza' : within(yard.bounds,x,z) ? 'yard' : wood && within(wood.bounds,x,z) ? 'nightwood' : butte && within(butte.bounds,x,z) ? 'mesa' : 'city';
    },
    // How high a step the ground allows here, where it differs from the default.
    stepHeightAt(x,z) {
      return within(plaza.bounds,x,z) ? PLAZA_STEP : undefined;
    },
    ...
    getNormal(x,z,out,feetY) {
      return within(plaza.bounds,x,z) ? (out.x=0,out.y=1,out.z=0,out) : navigation.getNormal(x,z,out,feetY);
    },
    // Fetches the detail that waits until the world is on screen: the plaza's
    // model. Its footprint is walked, and drawn, until it arrives.
    stream({ prepare } = {}) {
