...
try {
  ...
  world = await createWorld(scene, { lowPower: touch, nightwood: preferences.nightwood });
} catch (error) {
...
function openMenu(type){
  ...
  if(type==='settings'){
    ...
    // The world is built once, with or without the Nightwood: it takes a fresh one.
    $('nightwood-setting').checked=preferences.nightwood;$('nightwood-setting').onchange=e=>{preferences.nightwood=e.target.checked;save();location.reload();};
...
function animate(now){
  ...
  uiTime+=dt;if(uiTime>.15){uiTime=0;if(screen==='game'){updateHUD();drawMap();const landmark=world.landmarks.find(l=>Math.hypot(position.x-l.x,position.z-l.z)<20);$('region-name').textContent=landmark?landmark.name:{forest:'Pine Islet',plaza:'Lantern Plaza',yard:'Skibidi Yard',nightwood:'Nightwood Road'}[region()]||'City Quarter';$('world-clock').textContent=`${{forest:'ISLET',plaza:'PLAZA',yard:'YARD',nightwood:'WOODS'}[region()]||'CITY'} · ${formatTime(preferences.time)}`;}}
