...
try {
  ...
  world = await createWorld(scene, { lowPower: touch, nightwood: preferences.nightwood, mesa: preferences.mesa });
} catch (error) {
...
function openMenu(type){
  ...
  if(type==='settings'){
    ...
    // The world is built once, with or without the Nightwood and the mesa: it takes a fresh one.
    for(const district of ['nightwood','mesa']){$(district+'-setting').checked=preferences[district];$(district+'-setting').onchange=e=>{preferences[district]=e.target.checked;save();location.reload();};}
